...
export default {
      ...
      people: [
        {
          ...
          started: false,
          about:
            "This is an example Paper to demonstrate the functionality of the Webpage",
  ...
  watch: {
    indeterminate() {
      this.selectedPeople.values.length > 0 &&
        this.selectedPeople.values.length < this.people.length;
    }
  },
  methods: {
    time() {
    ...
    updateExam() {
      ...
      var personIdx = this.people.findIndex(
        (person) => person.id === this.people[thisid].id
      );
    ...
    addExam() {
      ...
      if (
        ...
        this.notification(
          "success",
          "Exam Added",
          this.newexamname + " has been added"
        );
      }
    },
    duplicateButton(personid: string) {
      ...
      this.notification(
        "success",
        "Exam Duplicated",
        this.people[personId].name + " has been duplicated"
      );
    ...
    deleteButton(personid: string) {
      ...
      this.notification(
        "success",
        "Exam Deleted",
        this.people[personIdx].name + " has been deleted"
      );
      // delete the person
    ...
    deleteSelected() {
      ...
      for (var i = 0; i < this.selectedPeople.length; i++) {
        // find the index of the person in the people array
        var personIdx = this.people.findIndex(
          (person) => person.id == this.selectedPeople[i]
        );
        // delete the person
        this.people.splice(personIdx, 1);
      }
      ...
      this.notification(
        "success",
        "Exams Deleted",
        "The selected exams have been deleted"
      );
      // also save it to local storage
      localStorage.setItem("people", JSON.stringify(this.people));
    },
    ...
    stopExam(personId: string) {
      ...
      this.notification(
        "success",
        "Exam Finished",
        this.people[personIdx].name + " has just finished"
      );
    ...
    startExam(personId: string) {
      ...
      if (this.people[personIdx].readingtimeenabled === true) {
        this.notification(
          "reading",
          "Reading Time Started",
          this.people[personIdx].name + " has started reading time"
        );
        var starttime = this.converter(
          this.accuratetime(),
          String(this.people[personIdx].readingtime),
          "addminutesexact"
        );
        this.people[personIdx].start = this.converter(
          this.time(),
          String(this.people[personIdx].readingtime),
          "addminutes"
        );
        var duration = this.converter(
          String(this.people[personIdx].duration),
          "",
          "H:MM"
        );
        // check if there is reading time and if so calculate the end time with the converter "addtime2"
        var durationandreading =
          Number(duration) + Number(this.people[personIdx].readingtime);
        this.people[personIdx].status = "reading";
        this.people[personIdx].end = this.converter(
          String(this.people[personIdx].start),
          String(duration),
          "addminutes"
        );
        var endtime = this.converter(
          String(starttime),
          String(duration),
          "addminutesexact"
        );
        ...
        this.notification(
          "success",
          "Exam Started",
          this.people[personIdx].name + " has been started"
        );
        this.people[personIdx].start = this.time();
        var durationinminutes = this.converter(
          String(this.people[personIdx].duration),
          "",
          "H:MM"
        );
          ...
          if (this.people[Number(personIdx)].status === "reading") {
            ...
            if (start == now) {
              ...
              this.notification(
                "success",
                "Exam Started",
                this.people[Number(personIdx)].name + " has been started"
              );
            }
          } else {
            var timeleft2 = this.converter(end, now, "timeleft");
    ...
    async calculateExtratime(personIdx: string, start: string, end: string) {
      ...
      this.notification(
        "extra",
        "Extra Time",
        this.people[Number(personIdx)].name + " has started extra time"
      );
      ...
      var extratimeend = this.converter(
        end,
        String(extratime4),
        "sectoendtime"
      );
              ...
              <div class="space-y-8 divide-y divide-gray-200">
                    ...
                    >
                      <div class="sm:col-span-4">
                        <label
                          ...
                          >Exam<a class="text-red-600"> *</a></label
                        >
                          ...
                          <div class="relative flex items-start">
                            <div class="flex h-5 items-center">
                              <input
                                ...
                                name="comments"
                                type="checkbox"
                                v-model="newexamextratimeenabled"
                            ...
                            <div class="ml-3 text-sm">
                              ...
                              <div class="mt-2">
                                ...
                                >
                                  <div
                                    v-if="newexamextratimeenabled"
                                    class="flex-auto items-center space-x-2"
                                  >
                                    <div class="mt-1 flex rounded-md shadow-sm">
                                      <input
                                        v-model="newexamextratime"
                                        placeholder="15"
                                        type="text"
                                        v-mask="'##'"
                                        name="examname"
                          ...
                          <div class="relative flex items-start">
                            <div class="flex h-5 items-center">
                              <input
                                id="candidates"
                                name="candidates"
                                v-model="newexamreadingtimeenabled"
                                type="checkbox"
                                class="h-4 w-4 rounded border-gray-300 text-orange-600 focus:ring-orange-500"
                              />
                            </div>
                            <div class="ml-3 text-sm">
                              ...
                              <div class="sm:col-span-2">
                                <div class="mt-2">
                                  ...
                                  >
                                    <div
                                      v-if="newexamreadingtimeenabled === true"
                                      class="mt-1 flex rounded-md shadow-sm"
                                    >
                                      <input
                                        v-model="newexamreadingtime"
                                        placeholder="15"
                                        type="text"
                                        v-mask="'##'"
                                        name="examname"
                ...
                <div class="pt-5">
                  <div class="flex justify-end">
                    ...
                    <button
                      @click="popupedit ? updateExam(people) : addExam()"
                    ...
                    >
                      {{ popupedit ? "Update Exam" : "Add Exam" }}
                    </button>
                  </div>
                </div>
              </div>
              ...
              <thead class="bg-gray-50">
                <tr>
                  <th scope="col" class="relative w-12 px-6 sm:w-16 sm:px-8">
                    ...
                    @click="selectAllPeople" :checked="selectedPeople.length ===
                    people.length">
                  </th>
                  <th
                    scope="col"
                    class="min-w-[12rem] py-3.5 pr-3 text-left text-md font-semibold text-gray-900"
                  >
                    Subject Name
                  </th>
                  <th
                ...
                >
                  <td class="relative w-12 px-6 sm:w-16 sm:px-8">
                    <div
                      ...
                      class="absolute inset-y-0 left-0 w-0.5 bg-orange-600"
                    ></div>
                    <input
                      type="checkbox"
                  ...
                  <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                    ...
                    >
                    <span
                      v-else-if="person.status === 'reading'"
                      class="inline-flex rounded-full bg-orange-100 px-2 text-xs font-semibold leading-5 text-orange-500"
                      >Reading Time</span
                    >
                    <span
                      v-else-if="person.status === 'extra'"
                      class="inline-flex rounded-full bg-orange-100 px-2 text-xs font-semibold leading-5 text-orange-500"
                      >Extra Time</span
                    >
                    <span
                      v-else-if="person.status === 'inactive'"
                      class="inline-flex rounded-full bg-red-100 px-2 text-xs font-semibold leading-5 text-red-800"
                      >Not Started</span
                    >
                    <span
                      v-else-if="person.status === 'finished'"
                  ...
                  >
                    <div class="inline-flex rounded-md shadow-sm">
                      <button
                        ...
                        "
                      >
                        {{ person.started ? "Stop Exam" : "Start Exam" }}
                      </button>
                        ...
                        >
                          ...
                          >
                            <div class="py-1">
                              ...
                              <MenuItem as="Delete" v-slot="{ active }">
                                <a
                              ...
                              <MenuItem as="Edit" v-slot="{ active }">
                                <a
