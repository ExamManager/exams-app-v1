...
export default {
  ...
  watch: {
    ...
    //         const containerOffset = document.getElementById("containerOffset")
  ...
  mounted() {
    ...
    this.days = this.genMonth(8, 2023).days;
    const testing = this.getExams(27, 7, 2023);

    console.log("testing getExams(27, 7, 2023)", testing);
  ...
  methods:
  {
    ...
    genMonth(month: number, year: number) {
      ...
      for (let d = 1 - firstDay; d <= daysInMonth + 6 - lastDay || newMonth.days.length < 42; d++)
      {
        ...
        newMonth.days.push({ date: `${tmp.getFullYear()}-${tmp.getMonth()+1}-${tmp.getDate()}`, isCurrentMonth: tmp.getMonth() == month, isToday: tmp.getDate() == currentDay && tmp.getMonth() == currentMonth && tmp.getFullYear() == currentYear, events: this.getExams(d, month, year) })
      }
    ...
    genWeek(day: number, month: number, year: number) {
      ...
      for (let d = day - providedDay; d < day - providedDay + 7; d++)
      {
        ...
        returnWeek.days.push({ date: `${tmp.getFullYear()}-${tmp.getMonth()+1}-${tmp.getDate()}`, isCurrentMonth: tmp.getMonth() == month, isToday: tmp.getDate() == day && tmp.getMonth() == month && tmp.getFullYear() == year, events: [] })
      }
    ...
    setExams() {
      ...
      const Exam1 = {
        start: new Date(2023, 6, 27, 9, 0),
        end: new Date(2023, 6, 27, 10, 0),
        extraTime: 25,
        readingTime: 10,
        subject: "Maths",
        yeargroup: "Year 11",
      ...
      const Exam2 = {
        start: new Date(2023, 6, 27, 10, 30),
      ...
      const Exam3 = {
        start: new Date(2023, 6, 27, 12, 0),
        end: new Date(2023, 6, 27, 13, 0),
        extraTime: 25,
        readingTime: 10,
        subject: "Maths",
        yeargroup: "Year 11",
      ...
      let exams = [exam1, exam2, exam3]
      // let exams = [this.objToExam(Exam1), this.objToExam(Exam2), this.objToExam(Exam3)]
    ...
    getExams(day: number, month: number, year: number) {
      ...
      // console.log("this.exams in getExams: ", this.exams[0])
...
<template>
    ...
    <!-- MONTH VIEW  -->
    <div class="h-screen overflow-y-scroll md:hidden" v-if="view == 1">
    ...
    <div class="h-screen flex-col hidden sm:flex" v-if="view == 1">
        ...
        <div ref="container" class="flex flex-auto flex-col overflow-auto">
          ...
          <div class="flex w-full flex-auto">
            ...
            <div class="grid flex-auto grid-cols-1 grid-rows-1">
              ...
              <ol v-for="event in selectedDay?.events" class="col-start-1 col-end-2 row-start-1 grid grid-cols-1" style="grid-template-rows: 1.75rem repeat(288, minmax(0, 1fr)) auto">
                <li class="relative mt-px flex" :style="{'grid-row': `74 / span ${6*1}`}">
                  <a href="#" class="group absolute inset-1 flex flex-col overflow-y-auto rounded-lg bg-blue-50 p-2 text-xs leading-5 hover:bg-blue-100">
                    <p class="order-1 font-semibold text-blue-700">Breakfast</p>
                    <p class="text-blue-500 group-hover:text-blue-700">
